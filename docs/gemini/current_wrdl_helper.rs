...
impl IntelligentSolver {
    ...
    pub fn get_best_guess(&self, remaining_words: &[String], guess_results: &[GuessResult]) -> Option<String> {
        ...
        // HYBRID OPTIMIZATION: Enhanced endgame strategy
        // When very few words remain, be more aggressive about guessing the answer
        if remaining_words.len() <= 2 {
            return remaining_words.first().cloned();
        }
        
        // CONSERVATIVE ENDGAME: Only when 2-3 words remain, be more strategic
        if remaining_words.len() <= 3 {
            // For very endgame, prioritize words that are more likely to be the answer
            // Use statistical analysis to pick the most probable word
            let mut best_endgame_word = None;
            let mut best_endgame_score = f64::NEG_INFINITY;
            
            for word in remaining_words.iter() {
                let statistical_score = self.calculate_statistical_score(word, remaining_words);
                // In endgame, statistical score is more important than entropy
                if statistical_score > best_endgame_score {
                    best_endgame_score = statistical_score;
                    best_endgame_word = Some(word.clone());
                }
            }
            
            if let Some(word) = best_endgame_word {
                return Some(word);
            }
        }
        ...
        for candidate in candidate_words.iter() {
            ...
            // CONSERVATIVE OPTIMIZATION: Moderate processing increase
            // Process up to 120 candidates (58ms → 150ms target allows moderate increase)
            if candidates_processed >= 120 {
                break;
            }
        }
    ...
    fn get_candidate_words(&self, remaining_words: &[String], _guess_results: &[GuessResult]) -> Vec<String> {
        ...
        // 3. CONSERVATIVE OPTIMIZATION: Maximum 250 candidates total
        // Using performance headroom more wisely - quality over quantity
        if candidates.len() > 250 {
